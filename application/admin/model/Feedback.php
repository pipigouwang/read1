<?php
/**
 * Created by PhpStorm.
 * User: Administrator
 * Date: 2018/8/8
 * Time: 13:37
 */

namespace app\admin\model;

use think\Db;
use think\Validate;
use think\Model;

class Feedback extends Model
{
    public function feedback_list($page,$limit,$con)
    {
        $con = array_filter($con);
        try{
            $res['list'] = Db::name('feedback')
                ->where($con)->page($page)->limit($limit)->select();
            if($res['list'])
            {
                $res['total'] = Db::name('feedback')
                    ->where($con)->count();
                return ['成功',7000,$res];
            }else{
                return ['暂无咨讯',-7001,['list'=>[],'total'=>0]];
            }
        }catch (\Exception $e)
        {
            return [$e->getMessage(),-7000,null];
        }
    }
    public function feedback_add($data)
    {
        $rule = [
            'text'=>'require',
        ];
        $msg = [
            'text.require'=>'内容不能为空',
        ];
        $data['create_time'] = date('Y-m-d H:i:s');
        $validate = new Validate($rule, $msg);
        $result   = $validate->check($data);
        if(!$result)
        {
            return [$validate->getError(),-7002,null];
        }
        try{
            $res = Db::name('feedback')
                ->insert($data);
            if($res)
            {
                return ['成功',7000,null];
            }else{
                return ['新增失败',-7003,null];
            }
        }catch (\Exception $e)
        {
            return [$e->getMessage(),-7004,null];
        }
    }
    public function feedback_edit($con,$data)
    {
        if(!isset($con['id'])||!Feedback::get($con['id']))
        {
            return ['未找到该咨讯',-7004,null];
        }
        $rule = [
            'text'=>'require',
            'status'=>'require|in:0,1,2'//0,删除，1，启用，2，禁用
        ];
        $msg = [
            'status.require'=>'状态必须',
            'text.require'=>'内容不能为空',
            'status.in'=>'非法状态',
        ];
        $validate = new Validate($rule, $msg);
        $result   = $validate->check($data);
        if(!$result)
        {
            return [$validate->getError(),-7002,null];
        }
        try{
            $res = Db::name('feedback')
                ->where('id','=',$con['id'])
                ->update($data);
            if($res)
            {
                return ['成功',7000,$res];
            }else{
                return ['修改失败',-7003,null];
            }
        }catch (\Exception $e)
        {
            return [$e->getMessage(),-7004,null];
        }
    }
    public function feedback_state($con,$data)
    {
        if(!isset($con['id'])||!Feedback::get($con['id']))
        {
            return ['未找到该咨讯',-7004,null];
        }
        try{
            $res = Db::name('feedback')
                ->where('id','=',$con['id'])
                ->update(['status'=>$data['status']]);
            if($res)
            {
                return ['成功',7000,$res];
            }else{
                return ['修改失败',-7003,null];
            }
        }catch (\Exception $e)
        {
            return [$e->getMessage(),-7005,null];
        }
    }
    public function feedback_one($con)
    {
        if(!isset($con['id'])||!Feedback::get($con['id']))
        {
            return ['未找到该咨讯',-7004,null];
        }
        try{
            $res = Db::name('feedback')
                ->where('id','=',$con['id'])
                ->find();
            if($res)
            {
                return ['成功',7000,$res];
            }else{
                return ['暂无数据',-7007,null];
            }
        }catch (\Exception $e)
        {
            return [$e->getMessage(),-7008,null];
        }
    }
}