...
class Feedback extends UserBase
{
    ...
    public function index()
    {
        ...
        $product = new \app\admin\model\Feedback();
        switch ($this->op)
        {
            case 'getlist':
                list($msg,$err,$data) = $product
                    ->feedback_list($this->page,$this->limit,$this->con);
                break;
            case 'getone':
                list($msg,$err,$data) = $product
                    ->feedback_one($this->con);
                break;
            case 'add':
                list($msg,$err,$data) = $product
                    ->feedback_add($this->data);
                break;
            case 'state':
                list($msg,$err,$data) = $product
                    ->feedback_state($this->con,$this->data);
                break;
            case 'update':
                list($msg,$err,$data) = $product
                    ->feedback_edit($this->con,$this->data);
                break;
        }
